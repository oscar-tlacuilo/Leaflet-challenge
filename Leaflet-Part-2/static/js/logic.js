// URLs for the earthquake and tectonic plates JSON data
...
d3.json(tectonicPlatesURL).then(function(data) {
    console.log("Tectonic Plates data:", data); // Log the tectonic plates data to the console

    L.geoJSON(data, {
        style: function() {
            return {color: "#FF4500", weight: 2};
        }
    }).addTo(tectonicPlates); // Add the tectonic plates layer to the map
    tectonicPlates.addTo(myMap); // Add the layer group to the map
}).catch(function(error) {
